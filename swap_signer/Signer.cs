...
namespace swap_signer
{
    public class Signer
    {
        //  origenal code is made by  Eng/ Bassem Aggamy     https://github.com/bassemAgmi/EInvoicingSigner
        //  modified by Eng Yasser Yousif 
        // for all vid see youtube https://www.youtube.com/channel/UCE9uYy3KeXvLl8_mUDi-sXQ
        //https://www.facebook.com/yaser.yousef.127


        private readonly string DllLibPath = "eps2003csp11.dll";
        private string TokenPin = "";
        private bool use_serial;
        private string cert_name = "";


        public static string add_signatuer(string JsonString, string Token_pin,bool using_serial,string certficate_name_serial)
        {
            ...
            // provide signer with pin and cert name ore serial and to serach by name or serial
            tokenSigner.TokenPin = Token_pin;
            tokenSigner.cert_name = certficate_name_serial;
            tokenSigner.use_serial = using_serial;
            ...
            else
            {
                try
                {
                    ...
                    if (cades == "No slots found" || cades == "No slots found")
                        return cades;
                }
                catch (Exception ex)
                {
        ...
        public static string add_pos_signatuer(string JsonString)
        {
            ...
            var head = request["header"] as JObject;
            JProperty dd = head.Property("uuid");

            string uuid = ComputeSha256Hash_static(canonicalString);

            dd.Value = uuid;


            // request.Add("signatures", signaturesArray);
            String fullSignedDocument = "{\"receipts\":[" + request.ToString() + "]}";
        ...
        public static string add_pos_uuid(string JsonString)
        {
        ...
        public string SignWithCMS(String serializedJson)
        {
            ...
            using (IPkcs11Library pkcs11Library = factories.Pkcs11LibraryFactory.LoadPkcs11Library(factories, DllLibPath, AppType.MultiThreaded))
            {
                ...
                using (var session = slot.OpenSession(SessionType.ReadWrite))
                {
                    ...
                    var foundCerts = store.Certificates.Find(X509FindType.FindByIssuerName, cert_name, false);
                    if (use_serial)
                    {
                        foundCerts = store.Certificates.Find(X509FindType.FindBySerialNumber, cert_name, true);
                    }
